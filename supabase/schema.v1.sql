...
create table if not exists public.admin_operation_events (
  ...
  action text not null check (action in ('dashboard_view', 'accommodation_approval', 'partner_inquiry_status', 'role_scope_check', 'qa_run')),
  target_type text not null check (target_type in ('admin_dashboard', 'accommodation', 'partner_inquiry', 'profile', 'booking', 'payment_order', 'utm_campaign')),
